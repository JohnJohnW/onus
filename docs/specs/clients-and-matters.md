...
- **Transitional ACIP relief (Transitional Rules ss7–8):** until **31 Mar 2029**, a
  ...
  procedures** for **nominated customer classes** instead of new initial CDD —
  provided that **by 1 July 2026** its policies list those classes + a stop-date per
...
Seed `monitoring_alert.indicator_key` across the 9 groups (Risk insights §4) — signals,
...
client_party                 -- BOs / controllers / agents / trust roles (s28(2)(b)-(d))
  ...
  ownership_pct numeric null, is_individual bool, details jsonb,   -- §4 schema if non-individual
...
matter
  id, firm_id, client_id, designated_service_key,   -- §3 catalogue
...
cdd_check
  ...
  kyc_fields jsonb,            -- §4 per-type schema
...
(Act s111; ss35E/35F) — see [reporting-and-recordkeeping.md](reporting-and-recordkeeping.md) §5.
...
`lpp_claimed` flag on the SMR — see reporting spec §5; it does not weaken s123.

## 12. Refer-outs — resolved

| Ref | Status |
|---|---|
| R1 designated services | ✅ Act s6 Tables 5 & 6 (§3) — in force in Comp 60 |
| R2 KYC field schema | ✅ Rules ss6-1…6-5 (§4) |
| R3 simplified/standard/enhanced | ✅ Act ss28/31/32; Rules 6-16…6-22 (§5) |
| R4 PEP | ✅ Rules s1-5, 6-23/6-24; Act s32(c) (§5) |
| R5 beneficial ownership | ✅ 25% or control; Rules 6-7/6-8 (§5) |
| R9 tipping-off + LPP | ✅ Act ss123, 242 (§11) |

**Residual (not blocking):** a **screening data provider** (sanctions/PEP/adverse-media —
e.g. OpenSanctions) — integration/licensing; the plain-**individual** minimum field set
